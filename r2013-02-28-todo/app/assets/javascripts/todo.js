...
function display_priority(priority)
{
  ...
  var img2 = $('<img>');
  img1.addClass('up');
  img1.attr('src', '/assets/famfamfam/icons/lightning_add.png')
  img2.addClass('down');
  img2.attr('src', '/assets/famfamfam/icons/lightning_delete.png')
...
// {  
//   // create new priority after json results, calls the display func. adds the json result to the adds priorities array. then empties out the current visiable array and then for each priority afterwards construct each of the following array. 
...
//  _.each(priorities, function(p){
  ...
  // img1.attr('src', '/assets/famfamfam/icons/lightning_add.png');
  // img2.addClass('down');
  // img2.attr('src', '/assests/famfamfam/icons/lightning_delete.png');
  // div1.append([img1, img2]);
...
//   div2.css('background-color', p.color);
//   div3.text(p.name);
//   div4.text(p.value);
//   div5.text(p.id);
